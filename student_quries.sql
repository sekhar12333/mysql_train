...
c. NULL : Does not require a value
...
g. UNSIGNED : Can not contain a negative value
...
n. PRIMARY KEY (SLIDE): Unique ID that is assigned to this row of data
    ...
    III. Must be given a value when the row is created and that value cant be NULL 
    
    IV. The original value can not be changed It should be short 
    
    V. It is probably best to auto increment the value of the key
...
Every table should focus on describing just one thing. Ex. Customer Table would have name, age, location, contact information. It should not contain lists of anything such as interests, job history, past address, products purchased, etc.
...
p. Some additional rules to help you make your data atomic: Do not have multiple columns with the same sort of information. Ex. If you wanted to include a employment history you should create job1, job2, job3 columns. Make a new table with that data instead. 

Do not include multiple values in one cell. Ex. You should not create a cell named jobs and then give it the value: McDonalds, Radio Shack, Walmart,¦ Normalized Tables

q. What does normalized mean?

Normalized just means that the database is organized in a way that is considered standardized by professional SQL programmers. So if someone new needs to work with the tables they will be able to understand how to easily.
...
You also want to eliminate using the same values repeatedly in your columns. Ex. You would not want a column named instructors, in which you hand typed in their names each time. You instead, should create an instructor table and link to its key. 
...
INT: A number with a value no bigger than 2^31 or smaller than 2^31 
...
15. CREATE TABLE test(
    ...
    V. A Foreign Key does not have to be unique
...
16. CREATE TABLE score(
    ...
    a. We combined the event and student id to make sure we do not have 
...
17. CREATE TABLE absence(
    ...
    a. Again we combine 2 items that arenot unique to generate a unique key
...
58. SELECT students.first_name, 
    ...
    b. Here Iam getting all the data for all quizzes and matching that 
...
61. Many-to-Many Relationship
    ...
    b. If a customer buys more than one product, you will have multiple product id is associated with each customer. As well, you would have multiple customer id is associated with each product.
